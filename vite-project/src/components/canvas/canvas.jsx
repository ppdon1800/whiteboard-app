...
function Canvas() {
    ...
    // make canvas sized to window initially (DPI-aware)
        ...
        const onMouseUp = () => {
            if (!mouseDown) return
            mouseDown = false
            const p = { color: currentColor.current, width: strokeWidth.current, path: currentPath }
    ...
    return (
        <div>
            ...
            <PenTool setPenOptions={setPenOptions} callBack={pickColor} callBack2={setStrokeWidth} eraser={Eraser} softEraser={SoftEraser} />
            <button onClick={()=>{Eraser.current = true;
                setPenOptions(false);
                SoftEraser.current = false;
            }}>Eraser</button>
            <button onClick={()=>{SoftEraser.current = true;
                setPenOptions(false);
